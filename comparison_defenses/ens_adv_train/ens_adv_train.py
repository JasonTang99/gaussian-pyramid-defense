...
def ens_adv_train(trainloader, criterion, optimizer, model, adv_models, writer, epoch, args):
    ...
    for i, (inputs_clean, targets_clean) in enumerate(tqdm(trainloader)):
        ...
        if args.attacker == 'stepll':
            ...
            # Step.L.L adv 
            inputs_adv = fast_gradient_method(adv_generating_model, inputs_clean, eps, ord=np.inf, clip_min= 0, clip_max= 1)            
